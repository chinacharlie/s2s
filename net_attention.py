...
class S2SNet:
    ...
    def get_inputs(self):
        x_mel = tf.placeholder(tf.float32, (hp.batch_size, hp.duration * 80, hp.n_mels), name = 'x_mel')
        x_spec = tf.placeholder(tf.float32, (hp.batch_size, hp.duration * 80, hp.n_fft // 2 + 1), name = 'x_spec')
        x_label = tf.placeholder(tf.float32, (hp.batch_size, None), name = 'x_label')
    ...
    def fnet(self, mel, is_training=True, reuse=None):
        ...
        out, _, _, _, _, _ = networks.decoder(self.x_mel, out, training=is_training)
        mid = out
